...
! ismabsoxygen
! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
! XSPEC local model for ISM absorption due to oxygen
! Version 1.0 July 2025 
!
! - This version only has CI-CVI as free parameters. 
! 
! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
subroutine ismabsoxygen(ear, ne, param, ifl, photar)
!
! The main routine to call all subroutines
!
implicit none
integer,parameter :: num_param = 9, out_unit=20, nion=8
...
version='1.0'
 if(startup)then
  print *, ' '
  print *, 'ISMabs_oxygen: ISM absorption model Version ',version 
  print *, 'NOTE: this model does not include hydrogen or helium, only O'       
  print *, ' '
  call read_cross_sections_ismabsoxygen( nemod,bxs,ifl)
  call create_energy_grid_ismabsoxygen(1.d1,1.d4,bener,nemod) !Absorption coefficient calculation grid  = cross section grid
  startup=.false.  
 endif
! Model parameters
N_N_1 = param(1)
...
call absorption_ismabsoxygen(N_N_1, N_N_2, &
N_N_3, N_N_4, N_N_5, N_N_6, N_N_7, N_N_8, & 
zfac, emod, nemod, coemod,bxs,cion,ifl,bener)

call map_to_grid_ismabsoxygen(dble(ear),ne,emod,nemod,photar,coemod,ifl)
return
end subroutine ismabsoxygen

! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++!
subroutine read_cross_sections_ismabsoxygen(bnene, xs, ifl)
  !---------------------------------------------------------------------
  ! Reads all cross sections and maps them onto a defined energy grid.
  ! Looks for the file at:
  !     <ismabsoxygen_root>/atomic_data/AtomicData.fits
  !---------------------------------------------------------------------

  implicit none
  integer, parameter :: nion = 8, out_unit = 20
  ...
  character(len=1024) :: local_dir1, local_dir2, ismabsoxygen_root, filename2
  ...
  local_dir2 = 'smabs.oxygen.dev'
  ismabsoxygen_root = trim(local_dir1) // trim(local_dir2)
  filename2 = trim(ismabsoxygen_root) // fileloc
...
end subroutine read_cross_sections_ismabsoxygen

! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++!
subroutine absorption_ismabsoxygen(N_N_1, N_N_2, N_N_3, N_N_4, N_N_5, N_N_6,  N_N_7, &
				  N_N_8, &
...
end subroutine absorption_ismabsoxygen

! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++!
subroutine map_to_grid_ismabsoxygen(new_en, nne, old_en, one, nflux, old_flu, ifl)
  ...
  do i = 1, nne
     ...
     call dbinsrch_ismabsoxygen(new_en(i - 1), bmin, old_en, one + 1)
     call dbinsrch_ismabsoxygen(new_en(i), bmax, old_en, one + 1)
     bmin = bmin - 1
     bmax = bmax - 1
...
end subroutine map_to_grid_ismabsoxygen

! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++!
subroutine dbinsrch_ismabsoxygen(e, k, ener, n)
  ...
  k = klo
end subroutine dbinsrch_ismabsoxygen

! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++!
subroutine create_energy_grid_ismabsoxygen(emin, emax, en, nen)
...
end subroutine create_energy_grid_ismabsoxygen
