...
! 
! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
subroutine ismabscalcium(ear, ne, param, ifl, photar)
!
! The main routine to call all subroutines
!
implicit none
integer,parameter :: num_param = 21, out_unit=20, nion=20
...
double precision :: N_N_17,N_N_18,N_N_19,N_N_20
...
version='1.0'
 if(startup)then
  print *, ' '
  print *, 'ISMabs_calcium: ISM absorption model Version ',version 
  print *, 'NOTE: this model does not include hydrogen or helium, only Ca'       
  print *, ' '
  call read_cross_sections_ismabscalcium( nemod,bxs,ifl)
  call create_energy_grid_ismabscalcium(1.d1,1.d4,bener,nemod) !Absorption coefficient calculation grid  = cross section grid
  startup=.false.  
 endif
! Model parameters
N_N_1 = param(1)
...
call absorption_ismabscalcium(N_N_1, N_N_2, &
...
call map_to_grid_ismabscalcium(dble(ear),ne,emod,nemod,photar,coemod,ifl)
return
end subroutine ismabscalcium

! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++!
subroutine read_cross_sections_ismabscalcium(bnene, xs, ifl)
  !---------------------------------------------------------------------
  ! Reads all cross sections and maps them onto a defined energy grid.
  ! Looks for the file at:
  !     <ismabscalcium_root>/atomic_data/AtomicData.fits
  !---------------------------------------------------------------------
  ...
  character(len=1024) :: local_dir1, local_dir2, ismabscalcium_root, filename2
  ...
  local_dir2 = 'smabs.calcium.dev'
  ismabscalcium_root = trim(local_dir1) // trim(local_dir2)
  filename2 = trim(ismabscalcium_root) // fileloc
...
end subroutine read_cross_sections_ismabscalcium

! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++!
subroutine absorption_ismabscalcium(N_N_1, N_N_2, N_N_3, N_N_4, N_N_5, N_N_6,  N_N_7, &
...
end subroutine absorption_ismabscalcium

! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++!
subroutine map_to_grid_ismabscalcium(new_en, nne, old_en, one, nflux, old_flu, ifl)
  ...
  do i = 1, nne
     ...
     call dbinsrch_ismabscalcium(new_en(i - 1), bmin, old_en, one + 1)
     call dbinsrch_ismabscalcium(new_en(i), bmax, old_en, one + 1)
     bmin = bmin - 1
     bmax = bmax - 1
...
end subroutine map_to_grid_ismabscalcium

! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++!
subroutine dbinsrch_ismabscalcium(e, k, ener, n)
  ...
  k = klo
end subroutine dbinsrch_ismabscalcium

! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++!
subroutine create_energy_grid_ismabscalcium(emin, emax, en, nen)
...
end subroutine create_energy_grid_ismabscalcium
